...
import { updateSiteService } from "@/src/server/services/sites/update-site";
import {
  siteIdParamsSchema,
  updateSiteSchema,
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const { id } = siteIdParamsSchema.parse(await context.params);

    const row = await getSiteService(auth.organizationId, id);
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const row = await updateSiteService({
